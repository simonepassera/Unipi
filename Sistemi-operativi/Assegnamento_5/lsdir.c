...
void lsR(const char nomedir[])
{
    ...
    else
    {
		...
		while((errno = 0, file = readdir(dir)) != NULL)
		{
	    	...
	    	strncpy(filename, nomedir, MAXFILENAME);
	    	strncat(filename, "/", MAXFILENAME - len1 - 1);
	    	strncat(filename, file->d_name, MAXFILENAME - len1 - 2);
...
int main(int argc, char *argv[])
{
    ...
    SYSCALL_EXIT(stat, r, stat(argv[1], &statbuf), "Facendo stat del nome %s: errno=%d\n", argv[1], errno);
