...
class CSP(Problem):
    ...
    def reset(self):
        """resets a CSP problem from one iteration to the next"""
        self.nassigns = 0
        self.curr_domains = None
        self.initial = ()
...
def degree(assignment, csp):
    ...
    return argmax(
        [v for v in csp.variables if v not in assignment],
        key=lambda var: len(csp.neighbors[var]))
...
def backtracking_search(csp,
    ...
    def backtrack(assignment):
        global trace
        if len(assignment) == len(csp.variables):   # complete assignement
            return assignment
        var = select_unassigned_variable(assignment, csp)
        for value in order_domain_values(var, assignment, csp):
            if 0 == csp.nconflicts(var, value, assignment):
                csp.assign(var, value, assignment)
                print ("Attempt " + var +"="+value)
...
def MapColoringCSP(colors, neighbors):
    ...
    dict of {region: [neighbor,...]} entries."""
    # variables are sorted to prevent different orderings of variables
    return CSP(sorted(neighbors.keys()), UniversalDict(colors), neighbors,
...
usa = MapColoringCSP(['R', 'G', 'B', 'Y'],
                     {'TN': ['MO', 'AR', 'MS', 'AL', 'KY', 'VA', 'NC', 'GA'], 'MA': ['NY', 'VT', 'NH', 'RI', 'CT'], 'KY': ['MO', 'IL', 'IN', 'OH', 'WV', 'VA', 'TN'], 'RI': ['MA', 'CT'], 'GA': ['AL', 'TN', 'NC', 'SC', 'FL'], 'WY': ['ID', 'UT', 'MT', 'SD', 'NE', 'CO'], 'LA': ['TX', 'AR', 'MS'], 'FL': ['AL', 'GA'], 'OK': ['CO', 'NM', 'KA', 'MO', 'AR', 'TX'], 'NE': ['WY', 'CO', 'SD', 'IA', 'MO', 'KA'], 'NH': ['VT', 'MA', 'ME'], 'DC': ['VA', 'MD'], 'MT': ['ID', 'ND', 'SD', 'WY'], 'MS': ['AR', 'LA', 'TN', 'AL'], 'MD': ['PA', 'WV', 'VA', 'DE', 'DC'], 'UT': ['NV', 'ID', 'WY', 'CO', 'AZ'], 'IA': ['SD', 'NE', 'MN', 'WI', 'IL', 'MO'], 'CA': ['OR', 'NV', 'AZ'], 'CO': ['UT', 'WY', 'NE', 'KA', 'OK', 'NM'], 'OH': ['IN', 'MI', 'PA', 'WV', 'KY'], 'NV': ['OR', 'CA', 'ID', 'UT', 'AZ'], 'NC': ['TN', 'GA', 'VA', 'SC'], 'WI': ['MN', 'IA', 'MI', 'IL'], 'SD': ['MT', 'WY', 'ND', 'MN', 'IA', 'NE'], 'DE': ['PA', 'NJ', 'MD'], 'NJ': ['PA', 'NY', 'DE'], 'NM': ['CO', 'OK', 'TX'], 'MO': ['NE', 'KA', 'OK', 'IA', 'IL', 'KY', 'TN', 'AR'], 'AZ': ['CA', 'NV', 'UT'], 'IN': ['IL', 'OH', 'KY', 'MI'], 'VT': ['NY', 'NH', 'MA'], 'TX': ['NM', 'OK', 'AR', 'LA'], 'ME': ['NH'], 'MI': ['WI', 'OH', 'IN'], 'CT': ['NY', 'MA', 'RI'], 'WA': ['OR', 'ID'], 'PA': ['OH', 'NY', 'NJ', 'DE', 'MD', 'WV'], 'IL': ['IA', 'MO', 'WI', 'IN', 'KY'], 'AR': ['OK', 'TX', 'MO', 'MS', 'TN', 'LA'], 'ID': ['WA', 'OR', 'NV', 'MT', 'WY', 'UT'], 'KA': ['CO', 'NE', 'MO', 'OK'], 'SC': ['GA', 'NC'], 'AL': ['MS', 'TN', 'GA', 'FL'], 'VA': ['KY', 'TN', 'WV', 'MD', 'DC', 'NC'], 'NY': ['PA', 'VT', 'MA', 'CT', 'NJ'], 'OR': ['WA', 'ID', 'NV', 'CA'], 'ND': ['MT', 'MN', 'SD'], 'MN': ['ND', 'SD', 'WI', 'IA'], 'WV': ['OH', 'KY', 'PA', 'MD', 'VA']})

france = MapColoringCSP(['R', 'G', 'B', 'Y'],
                         {'BR': ['NB', 'PL', 'NB', 'PL'],
                          'MP': ['AQ', 'AU', 'LI', 'LR', 'AQ', 'LI', 'AU', 'LR'],
                          'RA': ['AU', 'BO', 'FC', 'LR', 'PA', 'AU', 'BO', 'FC', 'PA', 'LR'],
                          'LR': ['AU','MP', 'AU', 'RA', 'PA', 'MP', 'PA', 'RA'],
                          'NB': ['BR', 'CE', 'NH', 'CE', 'PL', 'BR', 'NH', 'PL'],
                          'PI': ['CA', 'IF', 'NH', 'NO', 'NH', 'NO', 'CA', 'IF'],
                          'FC': ['AL', 'BO', 'CA', 'BO', 'CA', 'LO', 'AL', 'RA', 'LO', 'RA'],
                          'BO': ['AU', 'CE', 'IF', 'CA', 'FC', 'RA', 'AU', 'CA', 'CE', 'FC', 'IF', 'RA'],
                          'PL': ['BR', 'CE', 'NB', 'PC', 'BR', 'NB', 'CE', 'PC'],
                          'AQ': ['MP', 'LI', 'PC', 'LI', 'MP', 'PC'],
                          'PA': ['LR', 'LR', 'RA', 'RA'],
                          'AL': ['LO', 'FC', 'FC', 'LO'],
                          'CA': ['BO', 'IF', 'PI', 'LO', 'FC', 'BO', 'FC', 'IF', 'LO', 'PI'],
                          'CE': ['AU', 'BO', 'PL', 'NB', 'NH', 'IF', 'BO', 'AU', 'LI', 'PC', 'IF', 'LI', 'NB', 'NH', 'PC', 'PL'],
                          'LO': ['AL', 'CA', 'FC', 'CA', 'AL', 'FC'],
                          'PC': ['AQ', 'CE', 'LI', 'PL', 'CE', 'LI', 'AQ', 'PL'],
                          'NO': ['PI', 'PI'], 'NH': ['CE', 'IF', 'NB', 'PI', 'IF', 'CE', 'NB', 'PI'],
                          'IF': ['BO', 'CA', 'CE', 'NH', 'PI', 'CA', 'BO', 'CE', 'NH', 'PI'],
                          'LI': ['AQ', 'AU', 'CE', 'PC', 'CE', 'AU', 'MP', 'AQ', 'MP', 'PC'],
                          'AU': ['LI', 'CE', 'BO', 'RA', 'LR', 'MP', 'BO', 'CE', 'LI', 'LR', 'MP', 'RA']})
